...
class DatabaseService:
    ...
    async def get_user(self, user_id: int) -> Optional[User]:
        ...
        """
        with Session(self.engine) as session:
            user = session.get(User, user_id) # get object by primary key
            return user
    ...
    async def create_session(
        ...
        """
        with Session(self.engine) as session:
            chat_session = ChatSession(session_id=session_id, user_id=user_id, name=name, username=username)
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        """
        with Session(self.engine) as session:
            chat_session = session.exec(select(ChatSession).where(session_id == session_id)).first()
            if not chat_session:
                return False
    ...
    async def get_user_sessions(self, user_id: int) -> List[ChatSession]:
        ...
        with Session(self.engine) as session:
            statement = (
                select(ChatSession).where(col(ChatSession.user_id) == user_id).order_by(col(ChatSession.created_at)) # for type safety
            )
